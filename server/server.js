...
app.post("/send-booking-email", async (req, res) => {
  try {
    ...
    const sendSmtpEmail = {
      sender: { email: process.env.BREVO_SENDER_EMAIL, name: "Platypus- New Registration" },
      to: [{ email: "engkushpatelofficial@gmail.com" }],
  ...
  <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto; border: 1px solid #eee; border-radius: 8px; padding: 20px; background: #fafafa;">
      ...
      .map(
        (dog, i) => `
          <div style="margin-bottom: 10px; padding: 10px; border-left: 4px solid #3498db; background: #fff;">
            <p><strong>Dog ${i + 1}:</strong> ${dog.name}, ${dog.breed === "Other" ? dog.breedOther : dog.breed}, Age: ${dog.age || "?"}</p>
    ...
    <p><strong>Date:</strong> ${new Date(values.preferredDate).toDateString()}</p>
    ...
    <p>Vaccinations up to date: ${values.vaccinationsUpToDate ? "✅ Yes" : "❌ No"}</p>
